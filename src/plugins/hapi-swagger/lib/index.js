...
internals.getRoutesData = function (routes) {
    ...
    routes.forEach(function (route) {
        ...
        // version >= 4.x of Joi
        if (route.settings.validate && route.settings.validate.params) {
            routeData.pathParams = route.settings.validate.params
        }
...
internals.buildAPIInfo = function (settings, apiData, slug) {
    ...
    apiData.routes.forEach(function (route, indexA) {
        ...
        var pathParam = internals.getParamsBasedOnJOIVersion(route, 'pathParams')
        var queryParam = internals.getParamsBasedOnJOIVersion(route, 'queryParams')
        ...
        if (settings.payloadType === 'json') {
            ...
            var payloadParam = internals.getParamsBasedOnJOIVersion(route, 'payloadParams')
        ...
        var responseProperty = internals.validatorToProperty(responseClassName || op.nickname + '_response',
            internals.getParamsBasedOnJOIVersion(route, 'responseSchema'), swagger.models);
        if (responseProperty) {
...
// gets the pramas from version 2,3,4
internals.getParamsBasedOnJOIVersion = function (route, name) {
    var prama = route[name];
    // fixes for version >= 3.x of Joi
    if (route[name] && route[name].isJoi) {
        prama = route[name]._inner
        //fixes for version >= 4.x of Joi
...
internals.validatorToProperty = function (name, param, models, requiredArray) {
    ...
    if (param.describe) {
        ...
        if (describe.valid) {
            property.required = (describe.valids.indexOf(undefined) !== -1) ? false : true;
        }
        ...
        if (property.type === 'object' && param._inner) {
            ...
            // version >= 4.x of Joi - added children
